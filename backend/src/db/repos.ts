...
export async function setNicknameWithLimit(params: {
  tgUserId: string;
  nickname: string;
}): Promise<{ ok: boolean; remaining: number; gameNickname: string; gameUserId: string }> {
  ...
  try {
    ...
    const userRes = await client.query<{ game_nickname: string | null; game_user_id: string | null }>(
      `SELECT game_nickname, game_user_id FROM users WHERE tg_user_id = $1 LIMIT 1 FOR UPDATE`,
      [params.tgUserId],
    );
    ...
    const currentNickname = userRow.game_nickname == null ? null : String(userRow.game_nickname);
    const unchanged = currentNickname != null && currentNickname.toLowerCase() === normalized.lower;
    if (unchanged) {
      let existingGameUserId = userRow.game_user_id == null ? null : String(userRow.game_user_id);
      if (!existingGameUserId) {
        for (let attempt = 0; attempt < 10; attempt += 1) {
          const candidate = randomGameUserId();
          try {
            const idRes = await client.query<{ game_user_id: string }>(
              `
              UPDATE users
              SET game_user_id = $2,
                  updated_at = $3
              ...
              RETURNING game_user_id
              `,
              [params.tgUserId, candidate, now],
            );
            const maybeId = idRes.rows[0]?.game_user_id;
            if (maybeId) {
              existingGameUserId = String(maybeId);
              break;
            }
          } catch (error: any) {
            if (error?.code !== '23505') {
        ...
        if (!existingGameUserId) {
          const checkIdRes = await client.query<{ game_user_id: string | null }>(
            `SELECT game_user_id FROM users WHERE tg_user_id = $1 LIMIT 1`,
            [params.tgUserId],
          );
          existingGameUserId = checkIdRes.rows[0]?.game_user_id ? String(checkIdRes.rows[0].game_user_id) : null;
        }
      }

      await client.query('COMMIT');
      const remaining = await computeNameChangeRemaining(params.tgUserId);
      return {
        ok: true,
        remaining,
        gameNickname: currentNickname,
        gameUserId: String(existingGameUserId ?? ''),
      };
    }
    ...
    if (used >= 3) {
      ...
      return { ok: false, remaining: 0, gameNickname: currentNickname ?? '', gameUserId: String(userRow.game_user_id ?? '') };
    }

    let ensuredGameUserId = userRow.game_user_id == null ? null : String(userRow.game_user_id);
    if (!ensuredGameUserId) {
      for (let attempt = 0; attempt < 10; attempt += 1) {
        const candidate = randomGameUserId();
        try {
          const idRes = await client.query<{ game_user_id: string }>(
          ...
          const maybeId = idRes.rows[0]?.game_user_id;
          if (maybeId) {
            ensuredGameUserId = String(maybeId);
            break;
          }
        } catch (error: any) {
          if (error?.code !== '23505') {
      ...
      if (!ensuredGameUserId) {
        ...
        ensuredGameUserId = checkIdRes.rows[0]?.game_user_id ? String(checkIdRes.rows[0].game_user_id) : null;
      }
    }

    let updatedRow: { game_nickname: string; game_user_id: string | null } | null = null;
    try {
      const updateRes = await client.query<{ game_nickname: string; game_user_id: string | null }>(
        `
        UPDATE users
        SET game_nickname = $2,
            game_nickname_lower = $3,
            game_user_id = COALESCE(game_user_id, $4),
            updated_at = $5
        WHERE tg_user_id = $1
        RETURNING game_nickname, game_user_id
        `,
        [params.tgUserId, normalized.raw, normalized.lower, ensuredGameUserId ?? '', now],
      );
      updatedRow = updateRes.rows[0] ?? null;
    } catch (error: any) {
      if (error?.code === '23505') {
        const err = new Error('nickname_taken');
        (err as any).code = 'NICK_TAKEN';
    ...
    if (limitRes.rows[0]) {
      await client.query(
        `UPDATE user_name_limits SET change_count = LEAST(3, change_count + 1), updated_at = $3 WHERE tg_user_id = $1 AND day_key = $2`,
    ...
    return {
      ok: true,
      remaining,
      gameNickname: String(updatedRow.game_nickname),
      gameUserId: String(updatedRow.game_user_id ?? ensuredGameUserId ?? ''),
    };
  } catch (error) {
    await client.query('ROLLBACK');
    throw error;
...
export async function listLeaderboardTop(mode: string, limit: number): Promise<LeaderboardTopEntry[]> {
  ...
  const { rows } = await pgQuery<{
    ...
    SELECT
      ...
      ls.tg_user_id,
      COALESCE(NULLIF(u.game_nickname, ''), NULLIF(u.game_user_id, ''), 'Unknown') AS display_name,
      ls.best_score
