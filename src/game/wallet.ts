...
export async function checkMultiplayerResumeEligibility(): Promise<{ ok: boolean; eligible: boolean; roomCode?: string; matchId?: string; expiresAt?: number } | null> {
  ...
  if (!token) return null;
  try {
    const res = await fetch(apiUrl('/api/resume/eligibility'), { headers: { Authorization: `Bearer ${token}` } });
    const json = await res.json();
    if (!json?.ok) return null;
    return {
      ok: true,
...
export async function resumeMultiplayerSession(roomCode: string, matchId: string): Promise<{ ok: boolean; room?: RoomState; members?: RoomMember[]; matchId?: string; error?: string } | null> {
  const token = getToken();
  if (!token) return null;
  try {
    const res = await fetch(apiUrl('/api/resume/multiplayer'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ roomCode, matchId }),
    });
    const json = await res.json();
    if (!json?.ok) return { ok: false, error: String(json?.error ?? 'resume_failed') };
    return {
      ok: true,
      room: {
        roomCode: String(json.room?.roomCode ?? ''),
      ...
      members: Array.isArray(json.members) ? json.members.map((member: any) => ({ tgUserId: String(member?.tgUserId ?? ''), displayName: String(member?.displayName ?? 'Unknown'), joinedAt: String(member?.joinedAt ?? ''), ready: Boolean(member?.ready ?? false) })) : [],
...
export async function discardResume(): Promise<void> {
  const token = getToken();
  if (!token) return;
  try {
